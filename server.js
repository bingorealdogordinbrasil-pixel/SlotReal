...
mongoose.connect(MONGO_URI).then(() => console.log("✅ SISTEMA TOTALMENTE CONECTADO"));

// MODELO DE USUÁRIO COM CAMPO DE GANHOS
...
app.post('/auth/register', async (req, res) => {
    try {
        ...
        if(await User.findOne({ user })) return res.json({ success: false, message: "Usuário já existe!" });
...
// --- ROTA DE SAQUE (TRAVA DE SEGURANÇA) ---
app.post('/solicitar-saque', async (req, res) => {
    ...
    if (u && u.ganhos >= valor) {
        ...
        res.json({ success: false, message: `Limite de saque: R$ ${u ? u.ganhos.toFixed(2) : '0.00'} (Só ganhos de jogo são sacáveis)` });
    }
});

// --- ROTA DE GIRO (ATUALIZA SALDO E GANHOS) ---
app.post('/api/spin', async (req, res) => {
    ...
    const novoS = Number((u.saldo + ganho).toFixed(2));
    const novoG = Number((u.ganhos + ganho).toFixed(2));
...
app.post('/gerar-pix', (req, res) => {
    ...
    const data = JSON.stringify({
        ...
        payer: { email: `${userLogado.replace(/\s/g, '')}@gmail.com`, first_name: userLogado, last_name: "Jogador", identification: { type: "CPF", number: "19119119100" } }
    });
    const options = { hostname: 'api.mercadopago.com', path: '/v1/payments', method: 'POST', headers: { 'Authorization': `Bearer ${MP_TOKEN}`, 'Content-Type': 'application/json', 'X-Idempotency-Key': 'key' + Date.now() } };
