...
mongoose.connect(MONGO_URI).then(() => console.log("💎 SLOTREAL GOLD - SISTEMA INTEGRADO"));
...
// TIMER GLOBAL (Sincronizado para todos os jogadores)
let t = 30;
...
// SALVAR ESTADO (Saldo e Apostas Atuais)
app.post('/api/save-saldo', async (req, res) => {
    await User.findOneAndUpdate({ user: req.body.user }, { saldo: req.body.saldo, bets: req.body.bets });
    res.json({ success: true });
});

// LÓGICA DO GIRO (A roleta escolhe a cor onde houve MENOS aposta do usuário)
app.post('/api/spin', async (req, res) => {
    ...
    // Zera as apostas após o giro e atualiza saldo
...
app.post('/gerar-pix', (req, res) => {
    const postData = JSON.stringify({
        ...
        description: `Deposito_SlotReal_${req.body.user}`,
        payment_method_id: "pix",
        payer: {
            email: `${req.body.user}@gmail.com`,
        ...
        mpRes.on('end', () => {
            try {
                ...
                if (r.point_of_interaction) {
                    ...
                    res.json({ success: false, msg: "Erro na API do Mercado Pago" });
                }
            } catch (e) { res.json({ success: false }); }
        });
    });
...
app.post('/api/saque', async (req, res) => {
    ...
    const u = await User.findOne({ user: user });
    
    if (u && u.saldo >= valor && valor >= 20) {
        const novoSaldo = Number((u.saldo - valor).toFixed(2));
        await User.findOneAndUpdate({ user: user }, { saldo: novoSaldo });
        res.json({ success: true, novoSaldo: novoSaldo });
    } else {
        res.json({ success: false, msg: "Saldo insuficiente ou valor mínimo de R$ 20 não atingido." });
    }
});
