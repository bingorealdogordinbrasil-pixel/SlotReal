...
mongoose.connect(MONGO_URI).then(() => console.log("✅ BANCO E API CONECTADOS"));
...
app.post('/gerar-pix', (req, res) => {
    ...
    const cleanUser = String(userLogado || "usuario").replace(/[^a-zA-Z0-9]/g, '');
        ...
        mpRes.on('end', () => {
            try {
                ...
                if (r.point_of_interaction) {
                    ...
                    res.json({ success: false, message: "Erro MP: " + (r.message || "Verifique o Token") });
                }
            } catch (e) { res.json({ success: false, message: "Erro ao processar JSON" }); }
        });
    });
    mpReq.on('error', (e) => res.json({ success: false, message: "Erro de rede" }));
...
app.listen(PORT, () => console.log("🚀 SISTEMA SLOTGOLD ONLINE"));
